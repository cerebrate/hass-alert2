...
async def test_reminder(hass, service_calls):
    ...
    cfg = { 'alert2' : { 'alerts' : [
        ...
        { 'domain': 'test', 'name': 't3e', 'condition': 'sensor.b3', 'reminder_frequency_mins': [0.01, 0.05], 'reminder_message': 'yay-{{ on_secs }}-{{ on_time_str }}-{{ notify_reason }}', 'annotate_messages': False },
    ...
    service_calls.popNotifySearch('persistent_notification', 'yay', r'^yay-[12][.0-9]*-[12] s-ReminderOn$') # t3e
...
async def test_notifiers4(hass, service_calls):
    ...
    cfg = { 'alert2' : { 'defaults': { 'notifier': 'foo' }, 'alerts' : [
        ...
        { 'domain': 'test', 'name': 't9b', 'notifier': '{% if notify_reason == "Fire" %}foo{% else %}bar{% endif %}' },
    ]} }
    assert await async_setup_component(hass, DOMAIN, cfg)
    await hass.async_start()
    await hass.async_block_till_done()
    ...
    await hass.services.async_call('alert2','report', {'domain':'test','name':'t9b', 'message': 'zz'})
    await hass.async_block_till_done()
    service_calls.popNotifyEmpty('foo', r't9b: zz')
...
async def test_unack(hass, service_calls):
    cfg = { 'alert2' : { 'defaults': { 'reminder_frequency_mins': 0.01 }, 'alerts' : [
        ...
        { 'domain': 'test', 'name': 't45', 'ack_required': True, 'ack_reminder_message': '{{"ack-" + notify_reason}}-reminder' },
    ] } }
    hass.states.async_set("sensor.a", "off")
    assert await async_setup_component(hass, DOMAIN, cfg)
    await hass.async_start()
    ...
    service_calls.popNotifySearch('persistent_notification', 't45', 'ack-ReminderToAck-reminder')
...
async def test_new_vars(hass, service_calls):
    await setAndWait(hass, "sensor.a", 'off')
    cfg = { 'alert2' : { 'alerts': [
        { 'domain': 'test', 'name': 't1', 'condition': 'sensor.a', 'target': '{{ notify_reason }}t', 'title': '{{ notify_reason }}t2', 'message': '{{notify_reason}}-rm', 'done_message': '{{notify_reason}}-d' },
        { 'domain': 'test', 'name': 't2', 'trigger': [{'platform':'state','entity_id':'sensor.a'}], 'message': '{{notify_reason}}-tr' },
    ]}}
    assert await async_setup_component(hass, "notify", {})
    assert await async_setup_component(hass, "persistent_notification", {})
    assert await async_setup_component(hass, DOMAIN, cfg)
    ...
    assert service_calls.isEmpty()
    await setAndWait(hass, "sensor.a", 'on')
    service_calls.popNotifySearch('persistent_notification', 't1', 'test_t1: Fire-rm', extraFields={ 'target': 'Firet', 'title': 'Firet2' })
...
async def test_voice_proxies_default_off(hass, service_calls):
    ...
    switch_eid = a2Entities.getVoiceProxyEntityId('switch', 'test', 'voice0')
    assert hass.states.get(switch_eid) is None


async def test_voice_proxy_switch_condition_semantics(hass, service_calls):
    hass.states.async_set("sensor.a", "off")
    ...
    switch_eid = a2Entities.getVoiceProxyEntityId('switch', 'test', 'voice1')
    switch_ent = gad.voiceProxyEntityIdMap[switch_eid]
    ...
    attrs = hass.states.get(switch_eid).attributes
    assert attrs['alert_state'] == 'on'
    assert attrs['alert_firing'] is True
    assert attrs['alert_acked'] is False

    await switch_ent.async_turn_off()
    await hass.async_block_till_done()
    assert t1.is_acked()
    assert hass.states.get(switch_eid).state == 'off'
    assert hass.states.get(switch_eid).attributes['alert_acked'] is True
...
async def test_voice_proxy_event_latch_and_reload_toggle(hass, service_calls, monkeypatch):
    ...
    switch_eid = a2Entities.getVoiceProxyEntityId('switch', 'test', 'voice2')
    switch_ent = hass.data[DOMAIN].voiceProxyEntityIdMap[switch_eid]
    assert hass.states.get(switch_eid).state == 'off'
    ...
    assert attrs['alert_firing'] is True
    assert attrs['alert_acked'] is False
    assert attrs['event_latch_secs'] == 0.2
...
async def test_voice_proxy_per_alert_overrides(hass, service_calls, monkeypatch):
    ...
    v3a_switch = a2Entities.getVoiceProxyEntityId('switch', 'test', 'voice3a')
    assert hass.states.get(v3a_switch).state == 'off'

    v3b_switch = a2Entities.getVoiceProxyEntityId('switch', 'test', 'voice3b')
    assert hass.states.get(v3b_switch) is None
