...
* `alert2.alert2_global_exception` - This event alert fires if an HA task crashes due to an unhandled exception. This includes tasks for components outside Alert2.  For this alert, `throttle_fires_per_mins` defaults to `[20,60]` to limit alerts if some component goes into a crash loop. `exception_ignore_regexes` is also available to limit notifications from faulty components. See [Tracked](#tracked) section for an example.
...
| `voice_proxies_enabled` | bool | Enables Alexa-friendly voice switch proxy entities for that alert (`switch.<alert>`). Can be set in `defaults` and overridden per-alert. Defaults to `false`. |
| `voice_event_latch_secs` | float | For event alerts, controls how long the voice switch stays active after the most recent fire. Can be set in `defaults` and overridden per-alert. Minimum 0. Defaults to 60. |
...
| --&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`maximum` | float | optional | Maximum acceptable value for `value`. May be a float, template or entity name. At least one of `maximum` and `minimum` must be specified. |
| --&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`minimum` | float | optional | Minimum acceptable value for `value`. May be a float, template or entity name. At least one of `maximum` and `minimum` must be specified. |
| `delay_on_secs` | float | optional | Specifies number of seconds that any `condition` must be true and any threshold specified must be exceeded before the alert starts firing. Similar in motivation to the `skip_first` option in the old Alert integration. Can be a template. Default is 0. |
...
| `generator_name` | string | optional | Each generator creates a sensor entity with the name `sensor.alert2generator_[generator_name]`. See [generator patterns](#generator-patterns).<br>If `generator_name` is not specified, a name like "anonymous_nnn" will be used, where "nnn" is a positive integer. |
...
When an alert's effective `voice_proxies_enabled` is true (from `defaults` or a per-alert override), Alert2 creates one Alexa-friendly switch proxy in an Alexa-supported domain so that alert can be exposed and controlled via Home Assistant Cloud voice assistants.

- `switch.<alert>`
  - On while alert is firing and unacked.
    - Condition alerts: firing while state is `on`.
    - Event alerts: firing for `voice_event_latch_secs` after each fire.
  - Turn off while on: ack alert.
  - Turn on while off: unack alert only if still firing; otherwise no-op and remains off.

The proxy switch includes attributes (`alert_state`, `alert_firing`, `alert_acked`) for automations/debugging. Event alerts also expose `event_latch_secs` and `event_latch_remaining_secs`.

Expose this proxy switch (not the `alert2.*` entity) in Home Assistant Cloud for Alexa control.
